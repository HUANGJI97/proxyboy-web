 ...
 * 
 * 访问地址：
 * - http://115.159.196.184:8080/      -> 代理服务（默认）
...
const server = http.createServer((req, res) => {
  ...
  // ===== 代理服务（默认）=====
  // 如果不是 /admin 或 /cert 开头的请求，都作为代理处理
  handleProxy(req, res);
});
...
function handleProxy(req, res) {
  ...
  // 如果是 CONNECT 请求（HTTPS），交给 HTTPS 处理
  if (req.method === 'CONNECT') {
    res.writeHead(400);
    res.end('CONNECT method should be handled by CONNECT handler');
    return;
  }
...
server.listen(CONFIG.port, '0.0.0.0', () => {
  console.log(`代理+管理 服务启动成功！`);
  console.log(`监听端口: ${CONFIG.port}`);
  console.log(`\n访问地址:`);
  console.log(`  代理服务: http://115.159.196.184:${CONFIG.port}/`);
  console.log(`  Web 管理: http://115.159.196.184:${CONFIG.port}/admin`);
  console.log(`  证书下载: http://115.159.196.184:${CONFIG.port}/cert`);
  console.log(`\nCA 证书: ${CONFIG.caCertPath}`);
